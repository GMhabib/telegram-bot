...
const BOT_USERNAME = '@BOT_USERNAMEKAMU'; // Ganti dengan username bot Anda (misalnya, 'nama_bot')
...
function doPost(e) {
  try {
    ...
    if (contents.message) {
      ...
      if (text === '/start') {
      ...
      } else if (text.startsWith('/addadmin ') && isOwner(chatId)) {
        const adminIdToAdd = text.substring('/addadmin '.length).trim();
        const result = addAdmin(adminIdToAdd, chatId);
        sendMessage(chatId, result, 'HTML');
      } else if (text.startsWith('/removeadmin ') && isOwner(chatId)) {
        const adminIdToRemove = text.substring('/removeadmin '.length).trim();
        const result = removeAdmin(adminIdToRemove, chatId);
        sendMessage(chatId, result, 'HTML');
      ...
      } else if (text && text.startsWith('/kirim ')) {
        const connectedList = userConnections.getProperty(`admin_${chatId}_connections`);
        const messageToSend = text.substring('/kirim '.length);
        ...
        const isAdminOrOwner = isAdmin(chatId) || isOwner(chatId);
        const containsBannedKeywords = /(https?:\/\/[^\s]+)|(@[a-zA-Z0-9_]+)/i.test(messageToSend);

        if (containsBannedKeywords && !isAdminOrOwner && !isProtected(userId)) {
          // Implement permanent ban logic here
          const banDuration = 0; // 0 for permanent ban
          const banReason = 'Mengirim URL atau @username yang dilarang.';
          kickChatMember(chatId, userId); // Tendang pengguna dari grup (jika ada)
          blockUser(userId); // Blokir pengguna di bot
          sendMessage(chatId, `Pengguna dengan ID <code>${userId}</code> telah dibanned permanen karena mengirim URL atau @username yang dilarang.`);
          // Kirim notifikasi ke admin
          sendTelegramAdminMessage(`🚨 BAN OTOMATIS 🚨\nPengguna ID: <code>${userId}</code>\nNama: ${firstName}\nUsername: ${chatUsername}\nChat ID: <code>${chatId}</code>\nTelah dibanned permanen karena mengirim URL atau @username yang dilarang.`);
          return;
        }

        const connections = JSON.parse(connectedList);
        connections.forEach(targetId => {
          sendMessage(targetId, messageToSend);
        });
        sendMessage(chatId, `Pesan berhasil dikirim ke ID/Username: <code>${connections.join(', ')}</code>`, 'HTML');
