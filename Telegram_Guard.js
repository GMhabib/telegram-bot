  ...
  try {
    ...
    if (contents.message) {
      ...
      if (text === '/start') {
        let reply = `<b>👋 Halo!</b>\n\n` +
            `<b>Informasi Bot</b>\n` +
            `<b>ID Anda:</b> <code>${userId}</code>\n` + // Menampilkan ID pengguna sendiri masih berguna
      ...
      } else if (text.startsWith('/setowner ')) {
        if (isOwner(chatId)) {
          ...
          sendMessage(chatId, 'Anda bukan pemilik bot dan tidak memiliki izin untuk menggunakan perintah ini.', 'HTML');
        }
        ...
        // Pengguna masih bisa melihat ID mereka sendiri
      ...
      } else if (text === '/listblock' && isAdmin(chatId)) {
