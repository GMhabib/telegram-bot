...
const token = 'BOT_TOKEN_KAMU'; // Ganti dengan token bot Anda

const bot = new lumpia.init(token);

// masukkan ID kamu, jika belum tau cek di @strukturbot
// kunjungi url: https://sfl.gl/e4dc
const adminBot =   Admin_Bot_IDKamu; // Ganti dengan ID admin Anda
...
function handleBanCommand(chatId, messageId, text) {
  if (chatId !== adminBot) {
    bot.telegram.sendMessage(chatId, '🚫 Perintah ini hanya dapat digunakan oleh admin.', { reply_to_message_id: messageId });
    return;
  }

  const polaBan = /^\/ban (\d+)(?:\s+(.*))?$/i; // Match /ban <userid> [alasan]
  const cocok = polaBan.exec(text);

  if (cocok) {
    const userIdToBan = parseInt(cocok[1]);
    ...
    if (userIdToBan === chatId) { // Perbaiki: bandingkan dengan ID chat admin, bukan ID pesan
    ...
    try {
      ...
      const chatMemberBot = bot.telegram.getChatMember(targetChatId, bot.telegram.getMe().id);
      ...
      const banResult = bot.telegram.banChatMember(targetChatId, userIdToBan);
      if (banResult.ok) {
        bot.telegram.sendMessage(chatId, `✅ Berhasil memban pengguna ID <code>${userIdToBan}</code> di chat <code>${targetChatId}</code>.\nAlasan: ${htmlEntities(reason)}`, { parse_mode: 'HTML' });
        Logger.log(`User ${userIdToBan} banned by admin ${chatId} in chat ${targetChatId}. Reason: ${reason}`);
      } else {
        bot.telegram.sendMessage(chatId, `❌ Gagal memban pengguna ID <code>${userIdToBan}</code> di chat <code>${targetChatId}</code>.\nError: ${banResult.description || 'Unknown error'}`, { parse_mode: 'HTML', reply_to_message_id: messageId });
        Logger.log(`Failed to ban user ${userIdToBan} by admin ${chatId} in chat ${targetChatId}. Error: ${banResult.description}`);
      }
    } catch (error) {
      bot.telegram.sendMessage(chatId, `❌ Terjadi kesalahan saat mencoba memban pengguna ID <code>${userIdToBan}</code> di chat <code>${targetChatId}</code>: ${error.message}`, { parse_mode: 'HTML', reply_to_message_id: messageId });
      Logger.log(`Exception during ban command for ${userIdToBan} by admin ${chatId} in chat ${targetChatId}: ${error.message}`);
      // Pastikan bot adalah admin dengan hak ban
    }
  } else {
    bot.telegram.sendMessage(chatId, '❌ Penggunaan: /ban <user_id> [alasan]', { reply_to_message_id: messageId });
  }
}
// --- Fungsi RSS Feed ---
...
//      var userKey = `${chatId}_${userId}`; // Kunci unik per user per chat
      ...
      if (messageText) {
        ...
        // Command: /id
        if (/^\/id$/i.test(text)) {
          bot.telegram.sendMessage(chatId, `ID chat ini adalah: <code>${chatId}</code>`, { parse_mode: 'HTML', reply_to_message_id: messageId });
...
if (/^\/ban\s+\d+/i.test(text)) { // Deteksi awal format /ban
  handleBanCommand(chatId, messageId, text);
  return;
}
...
unban pengguna ID <code>${userIdToUnban}</code>.\nError: ${unbanResult.description || 'Unknown error'}`, { parse_mode: 'HTML' });
        ...
        if (cocok = polaShort.exec(text)) {
          ...
          var apiToken = 'SAFELINKU_API_TOKEN_KAMU'; // ⚠️ GANTI DENGAN API TOKEN SAFELINKU ANDA
...
function prosesCallback(cb) {
  try {
    if (/info_click/i.exec(cb.data)) {
      ...
      pesanMenu += "➡️ /ban &lt;user_id&gt; [alasan]\n";
      pesanMenu += "➡️ /unban &lt;user_id&gt;\n";
      pesanMenu += "\n<b>--- Perintah Admin ---</b>\n";
